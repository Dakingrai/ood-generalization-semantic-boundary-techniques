...
from src.process_sql import get_schema, Schema, get_sql
from src.exec_eval import eval_exec_match
...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    # sep_data = read_data('test_data/overall_data_iid.json')
    # inputs = []
    # predicted_natsql = []
    # gold_natsql = []
    # for i, line in enumerate(sep_data):
    #     inputs.append(line['input'])
    #     predicted_natsql.append(line['pred_editsql'])
    #     gold_natsql.append(line['gold_editsql'])

    # meta_data = read_data('test_data/overall_data_iid.json')
    ...
    # assert len(plist) == len(questions), "number of sessions must equal"
            ...
            if etype in ["all", "match"]:
                ...
                if exact_score == 0:
                    ...
                    turn_scores['exact'].append(1)
                        ...
                        # else:
                        #     print("idx: {}".format(count_q))
                        #     print("{} question: {}".format(hardness, input))
                        #     print("")
                        #     print("{} pred: {}".format(hardness, p_str))
                        #     print("{} gold: {}".format(hardness, g_str))
    ...
    print("###############")
    print(why_always)
    # with open("incorrect.txt", "w") as output:
    #     output.write(str(incorrect_idx))
    # print(incorrect_idx)
    with open('sep0.txt', 'w') as fout:
        json.dump(incorrect_idx, fout)
